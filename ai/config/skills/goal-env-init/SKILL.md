...
This is the **bootstrap command** — it must run before other `/ai-*` commands so that `ai/config/skills/` is copied into the project.

## Resolve PROJECT_ROOT and PLUGIN_ROOT

- `PROJECT_ROOT` — the directory containing both `ai/` and `modules/` (resolve via walk-up). On first init this directory may not yet have `ai/`; in that case `PROJECT_ROOT` is the current working directory where the user wants the project initialized.
- `PLUGIN_ROOT` — the installed plugin root: the parent of the `commands/` directory you loaded this skill's wrapper from (it also contains `skills/`, `agents/`, `ai/`). The agent knows this path. Pass it to `sync-templates.sh` as `$1`.
...
```bash
bash "${PROJECT_ROOT}/ai/config/skills/goal-env-init/scripts/init-dirs.sh"
```

(On first init, run from the desired project directory; the script resolves `PROJECT_ROOT` and creates `ai/`, `modules/`, `readonly-dependencies/` and all sub-dirs, then ensures `readonly-dependencies/*/*` is in `.gitignore`.)
...
```bash
bash "${PROJECT_ROOT}/ai/config/skills/goal-env-init/scripts/sync-templates.sh" "$PLUGIN_ROOT"
```
...
```bash
bash "${PROJECT_ROOT}/ai/config/skills/goal-env-init/scripts/materialize-repos.sh"
```
...
```bash
bash "${PROJECT_ROOT}/ai/config/skills/goal-env-init/scripts/scan-entries.sh"
```
