...
## 职责
领域核心：聚合根、值对象、领域服务、领域事件、Repository/消息端口接口。业务逻辑内聚在聚合根方法内。不感知 DB/MQ/三方服务。与业务中台 domain 的区别：**不调用扩展点**，业务变体差异以策略/参数显式处理。
...
## 依赖
- 可依赖：`clean-spring-ddd-starter`、`clean-spring-cqrs-starter`、`spring-context`/`spring-tx`
