...
## 职责
启动与装配：`Application` 主类、多环境配置（`application*.properties`/`logback-spring.xml`）、集成测试基类、代码生成器（将模板脚手架化为新微服务）。
...
- 【强制】依赖 `interface`（传递引入 application + infrastructure + facade + client）完成装配，是唯一可打包部署模块（`spring-boot-maven-plugin repackage`）。
- 【强制】一个微服务 = 一个这样的 starter 可部署单元；微服务间通过各自 `client` jar 的 `*OpenService` 通信。
...
## 依赖
- 可依赖：`interface`（传递 application/infrastructure/facade/client）、`spring-boot-starter`
- 角色：装配入口，本身不含业务逻辑
