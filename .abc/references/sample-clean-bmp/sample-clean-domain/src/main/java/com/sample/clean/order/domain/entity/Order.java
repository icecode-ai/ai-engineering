...
import com.icecode.clean.extension.ExtensionExecutor;
...
public class Order implements Aggregate<OrderId> {
    ...
    public void create(OrderRepository repository, OrderMessageProducer producer) {
        ...
        final OrderCreateInput input = new OrderCreateInput();
        input.setUserId(userId);
        input.setItemId(itemId.value());
        input.setItemTags(Lists.newArrayList(2));

        // 执行扩展点，写入业务属性
        this.attributes = ExtensionExecutor.executeFirstNotNull(OrderExtPt.class, ext -> ext.createAttributes(input));
