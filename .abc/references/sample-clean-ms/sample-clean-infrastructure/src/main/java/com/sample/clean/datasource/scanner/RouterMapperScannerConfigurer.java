 ...
 *
 * <p>
 * Copy from MapperScannerConfigurer by jim
 *
 ...
 */
public class RouterMapperScannerConfigurer
     ...
     *
     * @param excludeFilters list of TypeFilter
     *
     * @since 3.0.3
     */
     ...
     *
     * @param rawExcludeFilters list of rawExcludeFilter
     *
     * @since 3.0.3
     */
    ...
    public void postProcessBeanDefinitionRegistry(BeanDefinitionRegistry registry) {
        ...
        // update by jim
        var scanner = new ClasspathRouterMapperScanner(registry, getEnvironment());
    ...
    private void processPropertyPlaceHolders() {
        ...
        this.lazyInitialization = Optional.ofNullable(this.lazyInitialization)
            .map(getEnvironment()::resolvePlaceholders)
            .orElse(null);
        this.defaultScope = Optional.ofNullable(this.defaultScope).map(getEnvironment()::resolvePlaceholders).orElse(
            null);
    }
    ...
    private List<TypeFilter> mergeExcludeFilters() {
        ...
        try {
            for (Map<String, String> typeFilter : this.rawExcludeFilters) {
                typeFilters.add(createTypeFilter(
                    typeFilter.get("type"),
                    typeFilter.get("expression"),
                    this.getClass().getClassLoader()
                ));
            }
        } catch (ClassNotFoundException exception) {
            throw new RuntimeException(
                "ClassNotFoundException occur when to load the Specified excludeFilter classes.",
                exception
            );
        }
        return typeFilters;
    ...
    public void setMapperProperties(String[] properties) {
        ...
        for (String property : properties) {
            ...
            if (index < 0) {
                throw new MapperException("通过 @MapperScan 注解的 properties 参数配置出错:"
                    + property
                    + " !\n"
