...
public class InventoryModule {
    ...
    public InventoryDTO save(InventorySaveCommand command) {
        InventoryDO inventory = InventoryAssembler.INSTANCE.from(command);
    ...
    public Optional<InventoryDTO> query(InventoryQuery query) {
        return inventoryRepository.find(query.getItemId()).map(InventoryAssembler.INSTANCE::to);
    }

    @EventHandler(name = "库存扣减 - 订单消息监听")
    public void inventoryHandler(OrderEvent event) {
        Optional<InventoryDO> optional = inventoryRepository.find(event.itemId());
        Assert.isTrue(optional.isPresent(), "商品已下架");

        InventoryDO inventory = optional.get();
        inventory.setAvailableStock(inventory.getAvailableStock() - 1);
        Assert.isTrue(inventory.getAvailableStock() >= 0, "库存不足");
